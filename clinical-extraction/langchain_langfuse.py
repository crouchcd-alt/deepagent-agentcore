...
VCF_PATH = Path("../data/variants.vcf").resolve()
...
def _make_execute_tool(sandbox):
    """Create a LangChain tool that wraps the sandbox execute() method."""
    ...
    def execute(command: str) -> str:
        ...
        Usage:
        Executes a given command in the sandbox environment with proper handling
        and security measures.

        Before executing the command, please follow these steps:
        1. Directory Verification:
           - If the command will create new directories or files, first use
             execute("ls -la") to verify the parent directory exists and is the correct
             location.
        2. Command Execution:
        ...
        Usage notes:
          ...
          - When issuing multiple commands, use the ';' or '&&' operator to
            separate them. DO NOT use newlines (newlines are ok in quoted
            strings).
        ...
        Returns:
            A string with the command output and exit code.
        """
...
**guidelines:**
- Use the `execute` tool to run shell commands in the sandbox.
- Use `execute` to process the VCF files with scripts.
- To write files, use `execute` with a heredoc or python script that writes to disk.
- To read file contents, use `execute` with `python -c "..."` or write a script.
...
def run(sandbox, sandbox_infos):
    ...
    execute_tool = _make_execute_tool(sandbox)
    ...
    agent = create_agent(
        model=llm,
        tools=[execute_tool],
        system_prompt="You are a helpful data-processing assistant with access to a code sandbox.",
        middleware=[*agent_loggers],
    )
    ...
    result = agent.invoke(
        ...
        config={"callbacks": [langfuse_handler], "recursion_limit": 80},
    )
